	...
	function adjustFooterWidth( )
	{
		...
		if ( container )
		{
			...
			*/
			...
			{
				left: newLeft + 'px',
				right: (window.innerWidth - (rect.right + extraWidth)) + 'px',
				width: 'auto',
				marginLeft: '0',
	...
	*/
	...
	if ( allowedScrollbars.includes( codename ) )
	{
		...
		*/
		...
		document.body.replaceChildren( divPage );
		...
		*/
		...
		/*
			Hide footer spacer divs
		*/

		( function hideBottomSpacerIfPresent( )
		{
			const spacer = divPage.querySelector( '#ui-spacer-bottom' );
	...
	*/
	...
	if ( btnTheme )
	{
		...
		*/
		...
		function doUpdateThemeBtn( theme )
		{
			const icon = document.createElement( 'i' );
			icon.className = theme === 'dark' ? 'ax-solid ax-sun' : 'ax-solid ax-moon';

			btnTheme.replaceChildren( icon );
		...
		*/
		...
		btnTheme.addEventListener( 'click', ( ) =>
		{
			...
			*/
			...
			try
			{
				if ( window.top && window.top.document && window.top.document.documentElement )
					window.top.document.documentElement.classList.add( 'theme-transition' );
			} catch ( e ) { }
			...
			*/
			...
			{
				if ( window.top && window.top.document && window.top.document.documentElement )
					window.top.document.documentElement.setAttribute( 'data-theme', next );
			} catch ( e ) { }
			localStorage.setItem( 'theme', next );
			if ( codename === 'cyberpanel' )
				localStorage.setItem( 'cyberPanelTheme', next );
			doUpdateThemeBtn( next );
			...
			*/
			...
			setTimeout(( ) =>
			{
				...
				{
					if ( window.top && window.top.document && window.top.document.documentElement )
						window.top.document.documentElement.classList.remove( 'theme-transition' );
				} catch ( e ) { }
			}, 200);
		});
	}
	...
	/*
		Integration › CyberPanel › Theme Sync

		Cyberpanel contains its own theme button in header. We include one in the footer of CSF.
		Sync these buttons so that both of them change the theme for everythin together.
	*/
	...
	if ( codename === 'cyberpanel' )
	{
		( function ( )
		{
			function applyTheme( theme )
			{
				...
				{
					if ( window.top && window.top.document && window.top.document.documentElement )
						window.top.document.documentElement.setAttribute( 'data-theme', theme );
				} catch ( e ) { }
			}
			...
			*/
			...
			let theme = localStorage.getItem( 'cyberPanelTheme' ) || localStorage.getItem( 'theme' ) || 'light';
			applyTheme( theme );

			/*
				Listen for changes to localStorage (fires across tabs/windows)
			*/

			window.addEventListener( 'storage', ( env ) =>
			{
				if ( env.key === 'cyberPanelTheme' || env.key === 'theme' )
					applyTheme( env.newValue );
			});

			/*
				Poll for changes if host doesn't trigger 'storage'
			*/
			
			setInterval( ( ) =>
			{
				const current = localStorage.getItem( 'cyberPanelTheme' ) || localStorage.getItem( 'theme' ) || 'light';
	...
	*/
	...
	const footerButtons =
	{
		...
		'btn-blocklist': {
			...
			url:   'https://blocklist.configserver.dev/help'
		},
		'btn-status': {
			title: 'View Status',
			url:   'https://status.configserver.dev/status/services'
		},
		'btn-repo': {
	...
	for ( const [ id, { title, url } ] of Object.entries( footerButtons ) )
	{
		...
		if ( bDebug )
		{
			console.log( 'Setting button ' + title );
		}

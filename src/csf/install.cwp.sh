...
#   @description        CWP (CentOS Web Panel) Installer for Revolutionary Technology CSF/LFD
#
###############################################################################
...
# CWP is RHEL/CentOS based, so we expect yum/dnf
if [ -f /usr/bin/yum ]; then
    echo "    > Detected yum (RHEL/CentOS/CWP)."
    ...
    echo "    [WARN] Could not detect yum. Skipping dependency install (Is this CWP?)."
fi
...
# Load ECHO Module (Active Defense Target)
modprobe xt_ECHO 2>/dev/null
if lsmod | grep -q xt_ECHO; then
    echo "    [+] xt_ECHO module loaded (Target: ECHO available)."
else
    echo "    [-] xt_ECHO not loaded. (Kernel drops will use standard behavior)."
fi
...
# ==============================================================================
# PHASE 5: CWP CONTROL PANEL INTEGRATION
# ==============================================================================
if [ -d "/usr/local/cwpsrv" ]; then
    echo -n "Detected CWP... Installing module..."
    mkdir -p /usr/local/cwpsrv/htdocs/resources/admin/modules
    mkdir -p /usr/local/cwpsrv/htdocs/resources/admin/addons/ajax
    mkdir -p /usr/local/cwpsrv/htdocs/admin/design/csf
    
    cp -af cwp/csfofficial.php /usr/local/cwpsrv/htdocs/resources/admin/modules/
    cp -af cwp/csf.pl /usr/local/cwpsrv/htdocs/resources/admin/modules/
    cp -af cwp/ajax_csfframe.php /usr/local/cwpsrv/htdocs/resources/admin/addons/ajax/
    cp -af cwp/app.png /usr/local/cwpsrv/htdocs/admin/design/csf/
    
    chmod 755 /usr/local/cwpsrv/htdocs/resources/admin/modules/csf.pl
    chown -R cwpsrv:cwpsrv /usr/local/cwpsrv/htdocs/resources/admin/modules/csf.pl
    echo " done"
else
    echo " [WARN] CWP directory not found. Skipping UI integration."
fi

...
echo " Revolutionary Technology Enterprise Edition - cPanel"
echo ""
...
STREAMING_CHUNK: Integrating with cPanel/WHM AppConfig...

echo "Integrating with cPanel/WHM AppConfig..."
mkdir -p /usr/local/cpanel/whostmgr/docroot/cgi/
cp -af cpanel/csf.cgi /usr/local/cpanel/whostmgr/docroot/cgi/csf.cgi >/dev/null 2>&1 || true
cp -af cpanel/addon_csf.cgi /usr/local/cpanel/whostmgr/docroot/cgi/addon_csf.cgi >/dev/null 2>&1 || true
chmod 700 /usr/local/cpanel/whostmgr/docroot/cgi/csf.cgi >/dev/null 2>&1 || true
chmod 700 /usr/local/cpanel/whostmgr/docroot/cgi/addon_csf.cgi >/dev/null 2>&1 || true

if [ -x "/usr/local/cpanel/bin/register_appconfig" ] && [ -f "cpanel/csf.conf" ]; then
/usr/local/cpanel/bin/register_appconfig cpanel/csf.conf >/dev/null 2>&1 || true
fi

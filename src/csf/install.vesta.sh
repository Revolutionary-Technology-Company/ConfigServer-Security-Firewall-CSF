...
[ ! -d "/usr/local/vesta" ] && echo "FATAL ERROR: VestaCP not found." && exit 1

echo "Configuring ConfigServer Security and Firewall for VestaCP"
...
echo "Deploying VestaCP UI Plugin..."
mkdir -p /usr/local/vesta/web/list/csf
cp -af vesta/* /usr/local/vesta/web/list/csf/ >/dev/null 2>&1 || true
[ -x "/usr/local/csf/bin/auto.vesta.pl" ] && /usr/local/csf/bin/auto.vesta.pl || true

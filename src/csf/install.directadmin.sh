...
cp -af cwp/* /usr/local/cwpsrv/htdocs/resources/admin/modules/csf/ >/dev/null 2>&1 || true
chmod -R 755 /usr/local/cwpsrv/htdocs/resources/admin/modules/csf >/dev/null 2>&1 || true
if [ -f "/etc/csf/csf.conf" ]; then
sed -i 's/^GENERIC = "1"/GENERIC = "0"/' /etc/csf/csf.conf
sed -i 's/^CWP = "0"/CWP = "1"/' /etc/csf/csf.conf
fi"1"/' /etc/csf/csf.confistering systemd daemons...

le csf.service >/dev/null 2>&1 || true
